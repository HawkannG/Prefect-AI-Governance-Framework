...
│                                       References rules by policy area.
│                                       Human-owned. AI may suggest changes.
│
├── rules/
│   ├── policy.md                     ← Constitution (this file)
...
│   │                                     How features move from idea to code.
│   │                                     Human approves. AI follows.
│   │
│   ├── architecture.md               ← Project Structure
│   │                                     Approved directories and file locations.
│   │                                     Human approves. AI follows.
│   │
│   └── [additional rules]            ← Additional Rules (as needed)
│                                         Max 300 lines each.
│                                         Must reference a registered Policy Area.
│                                         Human approves creation. AI drafts content.
│
├── hooks/                            ← Enforcement Hooks
│   └── warden-*.sh                       Hard enforcement via Claude Code hooks
│
└── settings.json                     ← Hook Configuration
                                          permissions.deny for native protection
```
...
- To propose a new Policy Area, AI writes to .claude/rules/feedback.md with type "Policy Gap"
...
- Examples: `architecture.md`, `data-models.md`, `access-control.md` (in .claude/rules/)
...
2. AI writes a proposal to .claude/rules/feedback.md (type: "Directive Proposal")
...
- `.claude/rules/policy.md` (this file)
- `.claude/CLAUDE.md` (operating instructions)
- `.claude/rules/feedback.md` (feedback loop)
- `README.md` (project overview)
- `SECURITY.md` (security model documentation)
- `LICENSE` or `LICENSE.md` (project license)
- `lockdown.sh` (governance file lock/unlock utility)
- `.claude/` directory (governance files organized by official Claude Code structure)
...
- Source code files
- Directories not approved in .claude/rules/architecture.md
...
|------|-----------|---------------|
| `.claude/rules/policy.md` | 400 | Tighten language. Never expand to compensate. |
| `.claude/CLAUDE.md` | 80 | Move detail into rules. CLAUDE.md is an index. |
| `.claude/rules/feedback.md` | No limit, but max 20 open entries | Incorporate feedback, then mark resolved. |
| Any `.claude/rules/*.md` | 300 | Split into two rules files (requires human approval + Policy Area capacity). |
...
```
AI Detects Issue → Writes to .claude/rules/feedback.md
        ↓
...
### New Project
1. Copy .claude/ directory structure to your project
...
2. Edit .claude/CLAUDE.md: "Read .claude/rules/policy.md. Audit this project. Write drift report to .claude/rules/feedback.md."
...
*Changes to this constitution require a .claude/rules/feedback.md entry and human approval.*
