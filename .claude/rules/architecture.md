...
└── .claude/                     # Governance files (see .claude/rules/policy.md)
```
...
4. Write a proposal to .claude/rules/feedback.md (type: "Directive Gap")
...
- Only as last resort: request a cap increase via .claude/rules/feedback.md
...
If a naming conflict arises, document the resolution and rationale in .claude/rules/feedback.md for governance review.
...
When the approved tree is modified to add new directories for data models or schemas, document the change and its purpose in .claude/rules/feedback.md. If you create a data-models.md rule file (recommended when you have multiple models), update it to reflect the new model locations.

### INS-DA1-010: New Top-Level Directory Justification

When a new top-level directory is added (e.g., a new service, a new frontend app), document the rationale in .claude/rules/feedback.md, including:
...
- [ ] Root contains only allowed files (.claude/rules/policy.md Section 4)
